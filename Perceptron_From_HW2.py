...
def plot_separator(ax, th, th_0):
    ...
    if abs(th[1, 0]) > eps:
        pts += [np.array([x, (-th_0 - x * th[0, 0]) / th[1, 0]], dtype=object) for x in (xmin, xmax)]
    if abs(th[0, 0]) > 1.0e-6:
        pts += [np.array([(-th_0 - y * th[1, 0]) / th[0, 0], y], dtype=object) for y in (ymin, ymax)]
    in_pts = []
    for p in pts:
...
def perceptron(data, labels, params={}, hook=None):
    ...
    :param hook: Function to display separator, called with (th, th0) as a single parameter
    ...
    for tao in range(T):
        ...
        for i in range(num_samples):
            ...
            if current_guess <= 0:
                mistakes += 1       # use optionally if you need the mistake count
...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    :param labels: is numpy array of dimension 1 by n representing the labels of the samples
    :param params: Specifies the number of iterations T for the perceptron algorithm
    :param hook: Specifies the number of iterations T for the perceptron algorithm
    :return: A tuple of theta a d by 1 array and theta_0 a 1 by 1 array
    """
    # if T not in params, default to 100
    T = params.get('T', 100)
